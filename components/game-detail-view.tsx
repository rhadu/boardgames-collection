...
import { CheckCircle2, ExternalLink, Users, Clock, Calendar, Tag } from "lucide-react"
...
export function GameDetailView({
  ...
  const getConditionBadge = () => {
    if (game.condition === "Factory Sealed") {
      return (
        <Badge className="bg-sealed text-white shadow-lg border-0 px-3 py-1.5 text-sm">
          🎁 {t("sealed")}
        </Badge>
      )
    }
    if (game.condition === "Opened but Unplayed") {
      return (
        <Badge className="bg-primary text-primary-foreground shadow-lg border-0 px-3 py-1.5 text-sm">
          ✨ {t("unplayed")}
        </Badge>
      )
    }
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="w-full max-w-[calc(100vw-2rem)] lg:max-w-6xl max-h-[90vh] overflow-y-auto overflow-x-hidden xl:overflow-hidden p-0 gap-0">
          <DialogHeader className="sr-only">
            <DialogTitle>{game.title} - {t("gameDetails")}</DialogTitle>
          </DialogHeader>
              ...
              <div className="space-y-6 shrink-0">
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <h2 className="text-3xl font-bold mb-2 leading-tight">{game.title}</h2>
                  ...
                  <button
                    ...
                    aria-label={isSelected ? t("deselectGame") : t("selectGame")}
                  >
                ...
                <div className="flex items-center gap-2">
                  {getConditionBadge()}
                  {game.isKickstarter && (
                    <Badge className="bg-kickstarter text-white shadow-lg border-0 px-3 py-1.5 text-sm">
                      ⚡ {t("kickstarter")}
                    </Badge>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {game.players && (
                    <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
                      ...
                      <div>
                        <div className="text-xs text-muted-foreground uppercase tracking-wide">{t("players")}</div>
                  ...
                  {game.playtime && (
                    <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/50">
                      ...
                      <div>
                        <div className="text-xs text-muted-foreground uppercase tracking-wide">{t("playtime")}</div>
                ...
                {game.tags.length > 0 && (
                  <div>
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <h3 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">{t("tags")}</h3>
                    </div>
              ...
              <div className="space-y-6 shrink-0 mt-6">
                ...
                {game.notes && (
                  <div>
                    ...
                    <p className="text-sm leading-relaxed text-muted-foreground">{game.notes}</p>
                  </div>
                )}
                ...
                <div className="pt-6 border-t border-muted-foreground/20">
                  <div className="flex items-baseline gap-2 mb-6">
                    ...
                    <span className="text-lg text-muted-foreground">{game.currency}</span>
                  </div>
